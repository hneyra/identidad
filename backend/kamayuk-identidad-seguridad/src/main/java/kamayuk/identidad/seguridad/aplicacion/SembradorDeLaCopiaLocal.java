...
import kamayuk.identidad.seguridad.dominio.CatalogoDelSistema;
 ...
 *
 * <p>{@code rentas} tiene las once escrituras de administracion de seguridad —altas, bajas,
 * vigencias, excepciones por usuario— y con ellas su dominio, sus repositorios y sus pantallas. En
 * la <b>etapa 1</b> aqui <b>no hay ninguna</b>, y ese es el alcance del issue: el repositorio
 * existe, verifica y despliega. Lo que este sistema necesita hoy son exactamente dos cosas —leer
 * para autorizar, y sembrar al implantar—, asi que lo que hay es un lector ({@code
 * ComprobadorDeAccesoJdbc}) y este sembrador. La <b>etapa 2</b> es la que trae las once desde
 * {@code rentas}, y entonces esto deja de ser una copia local y pasa a ser el original (ADR-0039).
 *
 * <h2>La columna {@code sistema}, y lo que pasa sin ella</h2>
 *
 * <p>Este sembrador escribe {@code modulo_sistema} y {@code acceso} con {@link #SISTEMA} dentro,
 * porque el esquema de {@code identidad} llavea esas dos tablas por {@code (municipalidad_id,
 * sistema, codigo)} y no por {@code (municipalidad_id, codigo)}: aqui conviven los catalogos de los
 * cinco y dos sistemas pueden nombrar igual dos opciones distintas. Sin la columna, sembrar el
 * catalogo del segundo sistema chocaria con el UNIQUE del primero y el {@code ON CONFLICT ... DO
 * NOTHING} de abajo lo dejaria pasar <b>en silencio</b>: la opcion no se crearia, nadie podria
 * darle permiso, y el sintoma es una pantalla a la que no se le puede dar acceso.
 *
...
public class SembradorDeLaCopiaLocal extends RepositorioJdbc {
    ...
    /**
     * De que sistema son las opciones que este sembrador escribe.
     *
     * <p>Es una constante y no un campo de {@code CatalogoDelSistema.Opcion} <b>por ahora</b>: en
     * la etapa 1 el catalogo de esta base es solo el de {@code identidad}, asi que todas sus
     * opciones llevan el mismo valor y anadir el campo seria escribirlo seis veces. En la etapa 2,
     * cuando el catalogo pase a ser la union de los cinco, el sistema deja de ser una propiedad del
     * sembrador y pasa a serlo de cada opcion — y entonces esta constante tiene que desaparecer, no
     * quedarse como el valor por omision de las que no lo digan.
     */
    public static final String SISTEMA = "identidad";
    ...
    private long moduloId(CatalogoDelSistema.Opcion opcion) {
        ...
        // La lectura tambien lleva el sistema: sin el, `single()` revienta el dia que otro sistema
        // tenga un modulo con este mismo codigo — y `SEGURIDAD` es justamente el codigo que los
        // cinco usan.
        return jdbc().sql(
    ...
    private void otorgarLosSiete(long grupoId, String acceso, String quien) {
        ...
        jdbc().sql(
                        "INSERT INTO permiso (municipalidad_id, acceso_id, grupo_id,"
                                ...
                                + valores
                                + "   FROM acceso a"
                                + "  WHERE a.sistema = :sistema AND a.codigo = :acceso"
                                + "    AND NOT EXISTS ("
