...
/**
 * {@code identidad} sigue publicando lo que {@code normativa} le pide del buzon (ADR-0030 §4,
 * ADR-0039).
 *
 ...
 * POST /eventos/acuses}— siguen publicando todos los campos que el ingestor de {@code normativa}
 * lee y aceptando el cuerpo que manda. El rojo tiene que llegar <b>aqui</b> y no alli: quien retira
 * un campo de la respuesta es este repositorio, y con la prueba del lado del consumidor el aviso le
 * llegaria a quien no rompio nada.
 *
 * <h2>Por que esta DESACTIVADA hoy, y que la reactiva</h2>
 *
 * <p>Porque {@code normativa/docs/50-api/contratos-que-consume/identidad.json} <b>no existe
 ...
 * clones, asi que el dia que {@code normativa} lo publique esa guarda sale roja pidiendo que se
 * quite esta anotacion. Una prueba desactivada sin nada que la reactive es una prueba borrada con
 * mas lineas.
 */
@Disabled(
        "etapa 4: normativa no publica todavia docs/50-api/contratos-que-consume/identidad.json."
                + " Lo que exige quitar este @Disabled es ContratosDeLosConsumidoresTest, que se pone"
                + " roja el dia que el archivo aparezca")
@DisplayName("Contrato con normativa (identidad es el proveedor)")
class ContratoConNormativaTest extends ContratoConElConsumidorTestBase {
