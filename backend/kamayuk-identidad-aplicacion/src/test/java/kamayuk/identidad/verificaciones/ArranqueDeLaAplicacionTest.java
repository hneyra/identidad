...
import kamayuk.identidad.KamayukAplicacion;
...
/**
 * {@code identidad} arranca. Los dos perfiles, con el artefacto de verdad.
 *
 ...
 *
 * <ul>
 *   <li>Quitandole al modulo {@code kamayuk-identidad-seguridad} el {@code ComprobadorDeAccesoJdbc}
...
@DisplayName("C-7 — identidad arranca, en los dos perfiles")
class ArranqueDeLaAplicacionTest {
    ...
    void elPerfilWebLevanta(org.springframework.context.ApplicationContext contexto) {
        // No se pide ningun controlador, y no se puede: la etapa 1 no trae capa web (ver
        // `kamayuk.identidad.nucleo`). Lo que si tiene que estar es el puerto que el guardia pide
        // en CADA peticion, que es el bean que C-6 midio que faltaba en `catastro` y en `caja` —y
        // por el que esas dos aplicaciones no se habian arrancado nunca—.
