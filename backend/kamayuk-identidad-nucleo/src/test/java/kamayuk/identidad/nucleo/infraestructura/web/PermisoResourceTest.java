 ...
 *
 * <p>Y desde la etapa 2 comprueba una cosa mas: que el recurso dice <b>de que sistema</b> es la
 * opcion. Sin ese campo, «permisos» en la respuesta seria un codigo suelto y quien administra no
 * sabria si habla de la pantalla de este sistema o de la de {@code rentas}, que existen las dos.
 *
...
class PermisoResourceTest {
        ...
        PermisosController.PermisoResource deUsuario =
                PermisosController.PermisoResource.de(
                        new AdministrarPermisos.PermisoDeAcceso(
                                7L, "caja", "cierre_caja", null, 42L, Set.of(Privilegio.LECTURA)));
    ...
    void laFilaDeGrupoDiceSuGrupo() {
        PermisosController.PermisoResource deGrupo =
                PermisosController.PermisoResource.de(
                        new AdministrarPermisos.PermisoDeAcceso(
                                8L, "catastro", "calles", 3L, null, Set.of(Privilegio.LECTURA)));

        assertThat(deGrupo.grupoId()).isEqualTo(3L);
        assertThat(deGrupo.sistema())
                .as("el codigo solo no identifica una opcion en esta base: viaja con su sistema")
                .isEqualTo("catastro");
