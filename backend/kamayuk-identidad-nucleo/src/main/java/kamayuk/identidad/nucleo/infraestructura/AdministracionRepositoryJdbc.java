...
import kamayuk.identidad.nucleo.dominio.TipoDeAcceso;
import kamayuk.identidad.nucleo.dominio.Usuario;
import kamayuk.identidad.persistencia.OrdenSeguro;
...
public class AdministracionRepositoryJdbc extends RepositorioJdbc
    ...
    private static final OrdenSeguro ORDEN_MODULO =
            OrdenSeguro.sobre("codigo", "nombre", "orden", "sistema", "id").desempatandoPor("id");
    private static final OrdenSeguro ORDEN_ACCESO =
            OrdenSeguro.sobre("codigo", "nombre", "tipo", "sistema", "id").desempatandoPor("id");
    ...
    private static final String COLUMNAS_DE_ACCESO =
            "id, modulo_id, sistema, tipo, codigo, nombre, activo";
    ...
    /**
     * El filtro por sistema se compone y no se parametriza siempre, y por eso hay dos consultas.
     *
     * <p>Un {@code WHERE sistema = :sistema OR :sistema IS NULL} habria sido una sola, y es peor:
     * PostgreSQL no puede usar el indice de la clave para una condicion que depende de un parametro
     * nulo, asi que el listado de un sistema pasaria a leer el catalogo entero de los cinco. La
     * cadena que se concatena no viene del cliente —es {@code " WHERE sistema = :sistema"} o nada—
     * y el valor sigue viajando como parametro.
     */
    @Override
    public Pagina<Modulo> modulos(@Nullable String sistema, Paginacion paginacion) {
        String filtro = sistema == null ? "" : " WHERE sistema = :sistema";
        return paginar(
                "SELECT id, sistema, codigo, nombre, orden, activo FROM modulo_sistema" + filtro,
                "SELECT count(*) FROM modulo_sistema" + filtro,
                parametrosDe(sistema),
                paginacion,
                ORDEN_MODULO,
                AdministracionRepositoryJdbc::mapearModulo);
    }

    private static Map<String, Object> parametrosDe(@Nullable String sistema) {
        return sistema == null ? Map.of() : Map.of("sistema", sistema);
    }
    ...
    public Pagina<Acceso> accesos(@Nullable String sistema, Paginacion paginacion) {
        String filtro = sistema == null ? "" : " WHERE sistema = :sistema";
        return paginar(
                "SELECT " + COLUMNAS_DE_ACCESO + " FROM acceso" + filtro,
                "SELECT count(*) FROM acceso" + filtro,
                parametrosDe(sistema),
                paginacion,
                ORDEN_ACCESO,
    ...
    /**
     * La opcion por su par, que es la clave del esquema.
     *
     * <p><b>{@code single()} sobre el codigo solo reventaria</b>, y ese era el hueco que {@code V1}
     * dejo declarado: {@code permisos} es una opcion de {@code identidad} y otra de {@code rentas},
     * y con los cinco catalogos sembrados hay dos filas. Con el par hay una, siempre.
     */
    @Override
    public Optional<Acceso> accesoPorCodigo(String sistema, String codigo) {
        return jdbc().sql(
                        "SELECT "
                                + COLUMNAS_DE_ACCESO
                                + " FROM acceso WHERE sistema = :sistema AND codigo = :codigo")
    ...
    public Optional<Acceso> accesoPorId(long id) {
        return jdbc().sql("SELECT " + COLUMNAS_DE_ACCESO + " FROM acceso WHERE id = :id")
                .param("id", id)
                .query(AdministracionRepositoryJdbc::mapearAcceso)
                .optional();
    }
    ...
    public Miembro guardar(Miembro miembro) {
        jdbc().sql(
                ...
                .param("usuarioAlta", kamayuk.identidad.auditoria.OrigenContext.actual().usuario())
