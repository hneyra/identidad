...
public interface AdministracionRepository {
    ...
    /**
     * Los modulos del menu, opcionalmente acotados a un sistema.
     *
     * <p>El filtro es <b>opcional</b> y no obligatorio: quien compone el arbol de una municipalidad
     * los quiere todos —esta base guarda los cinco catalogos (ADR-0039)— y quien administra un
     * sistema concreto los quiere acotados. Nulo significa «los de los cinco», no «ninguno».
     */
    Pagina<Modulo> modulos(@Nullable String sistema, Paginacion paginacion);

    /** Las opciones, opcionalmente acotadas a un sistema. Mismo criterio que {@link #modulos}. */
    Pagina<Acceso> accesos(@Nullable String sistema, Paginacion paginacion);

    /**
     * La opcion, por su par {@code (sistema, codigo)}.
     *
     * <p><b>El sistema no es opcional aqui</b>, al reves que en los dos listados: {@code permisos}
     * es una opcion de {@code identidad} y otra de {@code rentas}, asi que buscar por el codigo
     * solo devolveria dos filas y elegir una seria resolver un permiso contra el catalogo
     * equivocado. La clave del esquema es {@code (municipalidad_id, sistema, codigo)} y esta firma
     * es esa clave.
     */
    Optional<Acceso> accesoPorCodigo(String sistema, String codigo);
