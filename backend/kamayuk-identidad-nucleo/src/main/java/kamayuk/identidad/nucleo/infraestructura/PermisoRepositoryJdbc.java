...
public class PermisoRepositoryJdbc extends RepositorioJdbc implements PermisoRepository {
    ...
    /**
     * Y de que sistema es. Es la mitad que en los otros cuatro no hace falta.
     *
     * <p>Esta base guarda los catalogos de los cinco, asi que hay <b>dos</b> filas con codigo
     * {@code permisos}: la de este sistema y la de {@code rentas}. Contar sin acotar daria por
     * administrador de {@code identidad} a quien administra los permisos de otro, y entonces la
     * guarda del ultimo administrador dejaria pasar el cambio que si deja el sistema sin quien
     * administre — y con la tranquilidad de haberlo comprobado.
     */
    static final String SISTEMA_DE_ADMINISTRACION = "identidad";
    ...
    private List<PermisoEfectivo> conOrigen(
        ...
        String sql =
                "SELECT a.sistema, a.codigo,"
                        ...
                        // Por el PAR, y no por el codigo: con los cinco catalogos sembrados
                        // `permisos` sale dos veces, y un orden que no las desempata deja dos
                        // paginas consecutivas repitiendo una fila y omitiendo la otra (#548).
                        + " ORDER BY a.sistema, a.codigo";
    ...
    public long usuariosQuePuedenAdministrarPermisos(LocalDate fecha) {
        ...
        return Objects.requireNonNull(
                jdbc().sql(sql)
                        .param("fecha", fecha)
                        .param("sistema", SISTEMA_DE_ADMINISTRACION)
