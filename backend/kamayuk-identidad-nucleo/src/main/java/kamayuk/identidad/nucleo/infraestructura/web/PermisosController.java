...
import kamayuk.identidad.nucleo.aplicacion.AdministrarPermisos.PermisoDeAcceso;
...
public class PermisosController {
    ...
    public List<PermisoResource> fijar(
        ...
        for (NivelDeAcceso nivel : nivelesDe(cambio.niveles())) {
            String sistema = sistemaDe(nivel);
            String acceso = accesoDe(nivel);
            Permiso permiso =
                    administrar.fijarParaGrupo(
                            grupo, sistema, acceso, privilegios(nivel), observacion);
            resultado.add(PermisoResource.de(permiso, sistema, acceso));
        }
        return resultado;
    }
    ...
    /**
     * De que sistema es la opcion de ese nivel, o 422.
     *
     * <p><b>No tiene valor por omision, y esa es la decision.</b> Suponer {@code identidad} haria
     * que un cuerpo escrito para {@code rentas} —donde el codigo {@code permisos} tambien existe—
     * fijara privilegios sobre la opcion equivocada, con 200 y sin que nada lo dijera. Es el mismo
     * caso que {@code privilegios} ausente: lo que no se puede adivinar se rechaza.
     */
    static String sistemaDe(NivelDeAcceso nivel) {
        String sistema = nivel.sistema();
        if (sistema == null || sistema.isBlank()) {
            throw new ProblemaDeNegocio(
                    CodigoDeError.VALIDACION,
                    "Cada nivel tiene que decir de que 'sistema' es su acceso: el codigo solo no"
                            + " identifica ninguna opcion, porque esta base guarda los catalogos de"
                            + " los cinco. Los cinco son "
    ...
    /**
     * Un acceso —por su <b>par</b>— y los privilegios que quedan otorgados sobre el.
     *
     * <p>Los tres campos van {@link Nullable} porque es lo que Jackson puede producir, y los tres
     * se rechazan con 422 nombrandose: ver {@link #sistemaDe}, {@link #accesoDe} y {@link
     * #privilegios}.
     */
    public record NivelDeAcceso(
            @Nullable String sistema,
            @Nullable String acceso,
            @Nullable List<String> privilegios) {}
    ...
    public record PermisoResource(
        ...
        static PermisoResource de(Permiso permiso, String sistema, String acceso) {
            List<String> nombres =
