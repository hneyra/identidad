...
import kamayuk.identidad.nucleo.dominio.Acceso;
import kamayuk.identidad.nucleo.dominio.AdministracionRepository;
...
import kamayuk.identidad.nucleo.dominio.Modulo;
...
public class AdministrarSeguridad {
    ...
    public AdministrarSeguridad(
            AdministracionRepository repositorio,
            Auditoria auditoria,
    ...
    /**
     * Los modulos del menu, de los cinco catalogos o de uno solo.
     *
     * <p>{@code sistema} nulo son los cinco, y es lo que necesita quien compone el arbol de una
     * municipalidad: esta base guarda a quien se le concede cada opcion de todos (ADR-0039).
     * Acotarlo es para quien administra un sistema concreto.
     */
    @Transactional(readOnly = true)
    public Pagina<Modulo> modulos(@Nullable String sistema, Paginacion paginacion) {
        return repositorio.modulos(sistema, paginacion);
    }

    /** Las opciones, de los cinco catalogos o de uno solo. Mismo criterio que {@link #modulos}. */
    @Transactional(readOnly = true)
    public Pagina<Acceso> accesos(@Nullable String sistema, Paginacion paginacion) {
        return repositorio.accesos(sistema, paginacion);
    }
     ...
     *
     * <p>No la publica ninguna ruta y no es codigo muerto: la usa {@link
     * kamayuk.identidad.nucleo.aplicacion.ImplantarMunicipalidad} para ser idempotente. El nombre
     * ya es unico por municipalidad en el esquema, asi que buscar por el es buscar por una clave, y
     * la alternativa —recorrer paginas— no lo seria.
     */
    @Transactional(readOnly = true)
    public Optional<Grupo> grupoPorNombre(String nombre) {
        return repositorio.grupoPorNombre(nombre);
    }

    /** La cuenta, si existe. Mismo motivo que {@link #grupoPorNombre}: la implantacion repite. */
    @Transactional(readOnly = true)
    public Optional<Usuario> usuarioPorCuenta(String cuenta) {
        return repositorio.usuarioPorCuenta(cuenta);
    }
    ...
    public Grupo inhabilitarGrupo(long id, Observacion observacion) {
        ...
        publicar(TipoDeEventoDeIdentidad.GRUPO_MODIFICADO, guardado);
        return guardado;
    }
    ...
    public Usuario inhabilitarUsuario(long id, Observacion observacion) {
        ...
        publicar(TipoDeEventoDeIdentidad.USUARIO_MODIFICADO, guardado);
        return guardado;
    }
    ...
    /**
     * Publica el hecho, <b>dentro de la misma transaccion</b> y despues de escribir la fila.
     *
     * <p>Las dos mitades importan. <b>Dentro</b>: sin {@code REQUIRES_NEW} y sin escucha del {@code
     * COMMIT}, de modo que una escritura que se deshace se lleva su evento — lo contrario deja al
     * consumidor aplicando un alta que aqui no existe, y nada lo dice. <b>Despues</b>: el hecho
     * lleva la fila TAL COMO QUEDO, con su identificador, y antes de escribir no lo hay.
     */
    private void publicar(TipoDeEventoDeIdentidad tipo, Grupo grupo) {
        buzon.emitir(HechoDeIdentidad.deGrupo(tipo, grupo));
    }

    private void publicar(TipoDeEventoDeIdentidad tipo, Usuario usuario) {
        buzon.emitir(HechoDeIdentidad.deUsuario(tipo, usuario));
    }

    /**
     * Quien hace el acto, del mismo sitio del que lo toma la auditoria.
     *
     * <p>No entra por la firma a proposito: seria un segundo sitio que dice quien hizo el cambio, y
     * el dia que los dos no coincidan la fila de {@code miembro} diria una cosa y la de {@code
     * auditoria} otra. Es el mismo argumento que {@code PermisoRepositoryJdbc} escribio para {@code
     * usuario_registro}.
     */
    ...
    /**
     * El grupo y el usuario, o 404. <b>Devuelve los dos</b> y no solo comprueba que existan.
     *
     * <p>El hecho que se publica lleva la clave natural de los dos —el nombre del grupo y la cuenta
     * del usuario— porque un identificador de esta base no significa nada en la del consumidor (ver
     * {@link HechoDeIdentidad#deMiembro}). Las dos filas ya se leian aqui para poder contestar 404,
     * asi que devolverlas no cuesta ninguna consulta: lo que costaba era descartarlas y volver a
     * pedirlas mas abajo.
     */
    private LasDosPartes exigirQueExistan(long grupoId, long usuarioId) {
        Grupo grupo = repositorio.grupo(grupoId).orElseThrow(() -> noEncontrado("grupo", grupoId));
        Usuario usuario =
                repositorio
                        .usuario(usuarioId)
                        .orElseThrow(() -> noEncontrado("usuario", usuarioId));
        return new LasDosPartes(grupo, usuario);
    }

    /** El grupo y el usuario de una afiliacion, leidos una sola vez. */
    private record LasDosPartes(Grupo grupo, Usuario usuario) {}

    private void publicar(Miembro miembro, LasDosPartes partes) {
        buzon.emitir(
                HechoDeIdentidad.deMiembro(
                        miembro, partes.grupo().nombre(), partes.usuario().cuenta(), quien()));
    }
