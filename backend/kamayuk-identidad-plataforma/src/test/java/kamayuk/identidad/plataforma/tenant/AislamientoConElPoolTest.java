package kamayuk.identidad.plataforma.tenant;
...
import kamayuk.identidad.esquema.ContextoDeTenant;
import kamayuk.identidad.esquema.DatosDePrueba;
...
class AislamientoConElPoolTest {
    ...
    void elContextoNoSobreviveALaDevolucionDeLaConexion() {
        try (Entorno entorno = new Entorno(1)) {
            long vistos = entorno.enContextoDe(municipalidadA, this::grupos);
            ...
            Throwable fallo = entorno.sinContexto(this::grupos);
    ...
    void dosMunicipalidadesSobreLaMismaConexionNoSeVenEntreSi() {
        try (Entorno entorno = new Entorno(1)) {
            assertThat(entorno.enContextoDe(municipalidadA, this::gruposDeA)).isEqualTo(1);
            assertThat(entorno.enContextoDe(municipalidadB, this::gruposDeA))
    ...
    void sinElGuardiaLaConexionContaminadaSeReutiliza() {
        ...
        try (Entorno entorno = new Entorno(1, false)) {
            ...
            long vistosSinContexto = entorno.enContextoDe(null, this::grupos);
    ...
    private long medir(Entorno entorno, int repeticiones) {
        ...
        for (int i = 0; i < repeticiones; i++) {
            entorno.enContextoDe(municipalidadA, this::grupos);
        }
        return System.nanoTime() - inicio;
    }

    private long grupos(JdbcTemplate jdbc) {
        Long total = jdbc.queryForObject("SELECT count(*) FROM grupo", Long.class);
        return total == null ? -1 : total;
    }

    private long gruposDeA(JdbcTemplate jdbc) {
    ...
    private long contarPor(JdbcTemplate jdbc, long municipalidadId) {
        Long total =
                jdbc.queryForObject(
                        "SELECT count(*) FROM grupo WHERE municipalidad_id = ?",
                        Long.class,
                        municipalidadId);
        return total == null ? -1 : total;
    }
