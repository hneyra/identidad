/* Comprueba que verificar-fila-del-registro.mjs muerde, y que no muerde de mas.
   ...
   tuvieron que anadir despues (`rentas`#45, `caja`#39):
     ...
     - `infrastructure/src/` tiene su muestra roja, y al lado una que toca
       `infrastructure/` FUERA de `src/` —su prueba y su README— y tiene que seguir
       pasando. Sin la segunda, «que el descriptor cuente» se podria satisfacer
       declarando que todo cuenta, y una guarda que grita en cada PR se acaba apagando;
     - `despliegue/` igual: su muestra roja y el contraste de `docs/` y `src/test/`.
...
const FILA = '| Lo que se verifico (#1, 3 pruebas) | La rotura | El rojo |';

const CASOS = [
  {
    nombre: 'cierra un issue, toca backend y NO deja fila',
    cuerpo: 'Cierra #1.\n\nLa etapa 1 de ADR-0039.',
  ...
  },
  {
    ...
    cuerpo: 'Closes #1',
    archivos: ['backend/kamayuk-identidad-seguridad/src/main/java/kamayuk/identidad/seguridad/X.java'],
    anadido: '+| Otra cosa (#2) | … | … |',
    esperado: 'rojo',
    dice: '#1',
  },
  ...
  {
    // `rentas`#45. El descriptor de despliegue decide que corre en la municipalidad, y alli
    // esta guarda no lo miraba: el mismo cambio salia `exit=0` en `rentas` y `exit=1` en los
    // otros tres. Aqui la ruta nace dentro, y esta muestra es lo que impide que se caiga.
    nombre: 'cierra un issue, toca el descriptor de despliegue y NO deja fila',
    cuerpo: 'Cierra #1.\n\nLa etiqueta `componente` de AC-7.',
    archivos: ['infrastructure/src/descriptor.ts'],
    anadido: '',
    esperado: 'rojo',
    dice: '#1',
  ...
  {
    // `caja`#39. El compose es la OTRA forma de levantar este sistema, y ADR-0011 tiene escrito
    // lo que cuesta que las dos se separen: «una variable nueva que entra en el cluster y no en
    // el compose». `rentas` lo dejo declarado como hueco y no cerrado; aqui entra desde el
    // primer commit.
  ...
  {
    // El contraste de las dos de arriba, y no es el mismo que el de `docs/`: estos dos archivos
    // viven DENTRO de `infrastructure/`, asi que separan «`src/` es codigo» de «todo
    // `infrastructure/` es codigo».
  ...
  },
  {
    ...
    archivos: [
      ...
      'backend/kamayuk-identidad-nucleo/src/test/java/kamayuk/identidad/nucleo/AlgoTest.java',
    ],
    anadido: '',
    esperado: 'verde',
  },
...
/* Y la direccion que faltaba, que es de `rentas`#45: TODO patron de `RUTAS_DE_CODIGO`
   tiene que tener al menos una muestra ROJA que lo ejerza.

   Sin ella, quitar una muestra no pone nada rojo: **deja de comprobarse, en verde**.
   Medido alli con la de `infrastructure/src/` fuera, la autoprueba decia «Las 7 muestras
   se comportan como deben» y salia con 0 — y peor, su contraste seguia ahi certificando
   que `infrastructure/` fuera de `src/` no cuenta, mientras nadie comprobaba que `src/`
   si. Es la leccion de «una regla sin muestra no protege nada» por el eje de las rutas.
...
const carpeta = mkdtempSync(join(tmpdir(), 'kamayuk-registro-'));
let fallos = 0;
