...
> los cinco sistemas van a tropezar con ellos exactamente igual. Un enlace a otro repositorio se
> deja de seguir; una copia se lee cuando duele.
>
> **Y aqui el hallazgo 1 pesa mas que en ningun otro repositorio**: esta es la base que guarda a
> quien se le concede cada opcion, asi que un rol que omita RLS no ve «filas de mas» — ve los
> permisos de todas las municipalidades a la vez, y con ellos a quien puede firmar que en cada
> una. Por eso `baseDeDatos()` del descriptor declara los tres roles con `superusuario: false` y
